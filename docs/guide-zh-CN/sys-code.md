    ...
    crud:
      templates:
        ...
        - group: "default"                                              # 分组名称
          ...
          apiPath: "./api/admingen"                                      # goApi生成路径
        ...
        - group: "addon"                                                # 分组名称
          ...
          masterPackage: "sys"                                          # 主包名称，需和controllerPath、logicPath、inputPath保持关联
          templatePath: "./resource/generate/default/curd"              # 模板路径
          apiPath: "./addons/{$name}/api/admingen"                      # goApi生成路径
...
- v3.0 起，DAO/DO/Entity 的生成已重构为基于自定义模板（`server/hack/tpl/`）的代码生成方式，替代了原有的内嵌 `gf gen dao` 方式，提供了更灵活的模板自定义能力。
...
- **DAO 模板文件**位于 `server/hack/tpl/` 目录：
  - `dao/dao.tpl` — 外层 DAO 模板（用户可自由修改扩展）
  - `dao/internal/dao_internal.tpl` — 内层 DAO 模板（自动生成，不建议手动修改）
  - `model/do/do.tpl` — DO 模型模板
  - `model/entity/entity.tpl` — Entity 模型模板
...
> v3.0 新增功能。当你需要清理已生成的代码文件时，可以使用一键清除功能，避免手动逐一删除文件。

**使用方式：**

1. 登录 HotGo 后台 → 开发工具 → 代码生成
2. 找到需要清除的生成记录，进入生成配置页面
3. 点击右上角的 **「清除代码」** 按钮（红色危险按钮）
4. 系统会自动预览所有已生成的文件列表，你可以选择需要删除的文件
5. 可选勾选 **「同时清除菜单权限」**，会递归删除生成代码关联的菜单及其所有子菜单
6. 确认后执行清除操作
...
> v3.0 起，代码生成器生成的文件统一增加 `.gen.` 标识后缀，例如：

| 文件类型 | 命名示例 |
|---------|---------|
| 路由文件 | `curd_demo.gen.go` |
| 控制器文件 | `curddemo.gen.go` |
...
> v3.0 新增功能。提供在线数据表结构管理能力，无需通过数据库客户端工具即可在后台完成表的创建和修改。

**功能入口：** 登录 HotGo 后台 → 开发工具 → 数据表管理

**支持的操作：**

| 操作 | 说明 |
|------|------|
| 查看表列表 | 查看指定数据库下的所有数据表，支持按表名搜索 |
| 查看表结构 | 查看数据表的字段详情、索引、注释等信息 |
| 创建数据表 | 通过可视化表单在线创建新的数据表 |
| 编辑表结构 | 在线修改表字段、索引等结构信息 |
| 删除数据表 | 删除指定数据表（危险操作，请谨慎使用） |
| DDL 预览 | 在执行表结构变更前，预览将要执行的 DDL 语句 |

**多数据库支持：** 页面顶部提供数据库切换选择器，可在配置的多个数据库之间切换管理。
...
- v3.0 起，DAO/DO/Entity 的生成已重构为基于 tpl 模板的独立实现（`gentpl` 包），不再依赖 `gf gen dao` 命令。模板文件位于 `server/hack/tpl/` 目录，支持完全自定义。

> **注意**：v3.0 起主模块代码生成不再生成 service 接口，Logic 层采用单例导出模式。Controller 直接调用 Logic 层，Service 层仅保留桥接接口用于打破循环依赖和多态注册。
