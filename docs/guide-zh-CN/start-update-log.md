...
- 重构：将 gen dao 重构为基于 tpl 的代码生成，新增独立包 `gentpl`，支持自定义 DAO/DO/Entity 模板（`server/hack/tpl/`），替代原有的内嵌 `gf gen dao` 方式
- 新增：代码生成增加一键清除功能，支持选择性删除已生成的代码文件，同时可清除关联的菜单权限（递归删除），带多重路径安全校验
- 新增：代码生成器文件名增加 `.gen.` 标识（如 `curd_demo.gen.go`），便于区分自动生成代码与手写代码
- 新增：数据表管理功能模块，支持在线查看/创建/编辑/删除数据表结构，支持 DDL 预览和多数据库切换
- 新增：菜单批量删除功能
- 新增：代码生成后自动执行 golangci-lint 检查（可选，默认开启）
- 优化：添加代码质量检查与 Git Hooks 配置（Husky + lint-staged + commitlint）
- 优化：统一 Less 导入语法为字符串格式
- 优化：更新 golangci-lint 配置并自动修复代码格式
- 优化：移除文件头部的版权信息注释
- 重构：v3.0 起主模块代码生成不再生成 Service 接口，Logic 层采用单例导出模式，Controller 直接调用 Logic
